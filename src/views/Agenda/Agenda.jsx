...
const Agenda = () => {
  ...
  const scrollDown = () => {
    episodeSection.current.scrollIntoView({
      behavior: "smooth",
      block: "start",
    });
  };
      ...
      <>
        <div className="container">
          <div className="agenda">
              ...
              <div className="flex flex-space-between full-width first-section">
                <div className="flex align-start column">
                  ...
                  <div className="big-text">
                    <div
                      dangerouslySetInnerHTML={{ __html: t("agenda.title") }}
                    ></div>
                  </div>
            ...
            <p className="paragraph">{t("agenda.text")}</p>
            <div className="space-100-height"></div>
            <div className="flex center">
              <div className="btn">
                <div className="btn-cover white-btn-cover big-btn-cover"></div>
                <button
                  className="big bordered big-white-border"
                  onClick={scrollDown}
                >
            ...
            {t("agenda.aboveText") && (
              <div className="small-text">
                <p>{t("agenda.aboveText")}</p>
              </div>
            )}
          </div>
        </div>
        <Tabs days={days} locations={locations}></Tabs>
        {t("agenda.bottomText") && (
          <div className="small-text">
            <p>{t("agenda.bottomText")}</p>
          </div>
        )}
        <div className="space-100-height"></div>
        <div className="space-85-height"></div>
  ...
  if (year >= 2023) {
    return (
      <>
        ...
        <iframe className="pretalx" style={{'border-width': 0}} width="100%" height="800" src="https://pretalx.herokuapp.com/datafest-2023/schedule" title=""></iframe>
            ...
            <div className="flex episodes">
              <div className="flex column col align-start">
                {days.map((day, dayIndex) => {
                  return (
                    <div className="flex align-start" key={dayIndex}>
                      <div className="small-text">
                        <p className="img-title margin-off left">
                          <span className="uppercase left">{day.date}</span>
                        </p>
                        ...
                        {day.agendaList.map((agenda, index) => {
                          return (
                            <div>
                              <div
                                className="flex align-start flex-start mobile-width"
                                key={index}
                              >
                                <p className="paragraph margin-off left">
                                  <span
                                    className={`time ${
                                      agenda?.key === "break" ? "pink-text" : ""
                                    } ${
                                      agenda?.key === "opening"
                                        ? "blue-text"
                                        : ""
                                ...
                                <div className="space-25-width"></div>
                                <p
                                  className={`paragraph margin-off left ${
                                    agenda?.key === "break" ? "pink-text" : ""
                                  } ${
                                    agenda?.key === "opening" ? "blue-text" : ""
                                  }`}
                                >
